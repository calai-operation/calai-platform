...
const recentEnough = (call) => {
  ...
  return (
    Number.isFinite(createdAt) && createdAt >= Date.now() - 4 * 60 * 60 * 1000
  );
};
...
export const reconcilePilotCalls = async () => {
  ...
  try {
    ...
    const linkedAssistantIds = new Set(
      linkedAgents.map((agent) => agent.vapiAgentId || agent.id),
    );
    const candidates = (Array.isArray(calls) ? calls : [])
      .filter(
        (call) =>
          call?.id &&
          call.status === "ended" &&
          linkedAssistantIds.has(call.assistantId) &&
          recentEnough(call) &&
          !processed.has(call.id),
      )
      .sort(
        (a, b) =>
          new Date(a.createdAt || a.startedAt) -
          new Date(b.createdAt || b.startedAt),
      );

    for (const call of candidates) {
      try {
        const result = await WebhookService.processVapiWebhook({
          message: { type: "end-of-call-report", call },
        });
        if (result?.success === false)
          throw new Error(result.message || "Call reconciliation was rejected");
        processed.add(call.id);
      } catch (error) {
        console.error(
          `[PilotCallReconciler] Failed for call ${call.id}:`,
          error.message,
        );
      }
    }
    if (candidates.length)
      console.log(
        `[PilotCallReconciler] Checked ${candidates.length} ended pilot call(s).`,
      );
  } catch (error) {
