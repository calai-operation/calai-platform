...
export function buildOrderEmail({ businessName, order, callbackNumber }) {
  const pending = order.confirmationStatus === "unconfirmed",
    status = pending ? "Unconfirmed Order" : "Order Confirmed",
    accent = pending ? "#e88722" : "#008fe8";
  const reference = String(order.id || "").slice(0, 8),
  ...
  const date = order.createdAt
    ...
    : "Not recorded";
  const details = [
    ["Order reference", reference ? "#" + reference : "Not recorded"],
    ["Business", business],
    ["Received", date],
    ["Order type", type],
    ["Customer", order.customerName || "Not provided"],
    ["Contact number", callbackNumber || "Not provided"],
  ];
  if (order.orderType === "DELIVERY")
    details.push(["Delivery address", order.deliveryAddress || "Not provided"]);
  if (order.pickupTime) details.push(["Collection time", order.pickupTime]);
  const reason =
    order.unconfirmedReason || "The call ended before the order was confirmed.";
  const intro = pending
    ? "An order was captured but the customer did not complete confirmation. Please review the details and contact the customer before preparing it."
    : "A customer has confirmed a new order through Calai. The details are below.";
  ...
  const text = [
    ...
    ...rows.map(
      (r) =>
        `${r.qty} × ${r.name} — ${price(r.total)}${r.notes ? "\nNote: " + r.notes : ""}`,
    ),
    ...(notes ? ["", "Order notes: " + notes] : []),
    "",
    "Total: " + price(order.totalPrice),
    ...(pending ? ["", "Reason not confirmed: " + reason] : []),
    "",
    "View orders: https://calai.info/owner/order-list",
    "Need help? Contact hello@calai.info",
    "Calai · Business order notifications",
  ].join("\n");
  ...
  return {
    subject: `Calai | ${status}${reference ? " #" + reference : ""}`,
    text,
    html,
  };
}

export function createOrderEmailSender({
  prisma,
  sendEmail,
  logger = console,
}) {
    ...
    try {
      ...
      await sendEmail({
        ...
        ...buildOrderEmail({
          ...
          order,
          callbackNumber: call?.customerNumber,
        }),
      });
    } catch (error) {
      logger.error(
        "Order notification email could not be sent:",
        error.message,
      );
    }
  };
