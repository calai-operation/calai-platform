...
export const CARD_TRANSFER_MESSAGE =
  "Your order's been confirmed. I'll transfer you to the restaurant now to complete the card payment.";
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
export const isDefiniteTransferRejection = (status) =>
  [400, 401, 403, 404, 405, 410, 415, 422, 429].includes(status);
const failed = () => ({
  success: false,
  status: "not_initiated",
  message:
    "The payment transfer was not initiated. Explain the problem and offer the supported staff-transfer option. Do not save the order again or claim payment is complete.",
});
const uncertain = () => ({
  success: false,
  status: "unknown",
  message:
    "The transfer request was sent, but its outcome could not be verified. It may already be connecting. Do not retry or call another transfer tool. Keep the caller informed without claiming payment is complete.",
});
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(
          () => reject(Error("Transfer state unavailable")),
          2000,
        );
      }),
    ]);
  ...
  return {
    async claim(id) {
      ...
      return {
        acquired: false,
        result:
          prior?.status === "initiated"
            ? { success: true, status: "initiated", message: prior.message }
            : uncertain(),
      };
    },
    async finish(id, result, lease) {
      ...
      const script =
        "if redis.call('GET',KEYS[1]) ~= ARGV[1] then return 0 end if ARGV[2] == 'not_initiated' then return redis.call('DEL',KEYS[1]) end return redis.call('SET',KEYS[1],ARGV[3],'EX',86400)";
      await bounded(
        redis.eval(script, {
          keys: [key(id)],
          arguments: [lease, result.status, JSON.stringify(result)],
        }),
      );
    },
  };
...
export function createCardPaymentTransfer({
  getCall,
  getAssistant,
  getTool,
  findOrder,
    ...
    const run = (async () => {
      ...
      try {
        ...
        const tools = [
          ...
          ...(await Promise.all(
            (assistant.model?.toolIds || []).map((id) =>
              getTool(id, { signal }),
            ),
          )),
        ];
        const destinations = tools
          .filter((tool) => tool?.type === "transferCall")
          .flatMap((tool) => tool.destinations || []);
        if (
          destinations.length !== 1 ||
          destinations[0].type !== "number" ||
          !/^\+[1-9]\d{6,14}$/.test(destinations[0].number || "")
        )
          return failed();
        const url = new URL(call.monitor?.controlUrl);
        if (
          url.protocol !== "https:" ||
          !url.hostname.endsWith(".vapi.ai") ||
          url.username ||
          url.password ||
        ...
        await transfer(
          url.href,
          { type: "transfer", destination, content: CARD_TRANSFER_MESSAGE },
          { signal },
        );
        result = {
          success: true,
          status: "initiated",
          message:
            "Payment transfer initiated. The saved order is confirmed; payment itself is not complete. Do not initiate another transfer.",
        };
      } catch (error) {
        // Only an explicit HTTP rejection proves the transfer was not accepted.
        result =
          dispatched && !error?.definitelyNotInitiated ? uncertain() : failed();
      }
      if (claim?.acquired) {
        try {
          await attemptStore.finish(callId, result, claim.lease);
        } catch {
          /* Keep the durable unknown marker if final-state storage fails. */
        }
      }
      return result;
    })();
...
export function createCardPaymentTransferController({ getSecret, handle }) {
  return async function cardPaymentTransferController(req, res) {
    ...
    if (
      ...
      calls.length !== 1 ||
      !calls[0]?.id ||
      calls[0].function?.name !== "transferForCardPayment"
    )
    ...
    return res
      .status(200)
      .json({
        results: [{ toolCallId: calls[0].id, result: JSON.stringify(result) }],
      });
  };
}
