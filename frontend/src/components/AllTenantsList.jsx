...
const AllTenantsList = ({ tenants = [] }) => {
  ...
  const tableHeads = [
    { 
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#0F172A] border border-[#1e293b] flex items-center justify-center text-[#3b82f6] font-medium text-sm shrink-0">
            {row.tenant.initials}
          </div>
          <div>
            <div className="text-[14px] font-semibold text-white mb-0.5">{row.tenant.name}</div>
    ...
    { 
      ...
      sortable: true,
      render: (row) => (
        <span className="inline-flex items-center gap-1.5 bg-[#381B20] text-[#E74C3C] text-[11px] font-bold px-2.5 py-1 rounded-md">
    ...
    },
    {
      ...
      render: () => (
        <div className="flex justify-end pr-4">
          <Icon icon="lucide:arrow-up-right" className="text-gray-400 w-4 h-4 cursor-pointer hover:text-white transition-colors" />
        </div>
      )
    }
  ];
